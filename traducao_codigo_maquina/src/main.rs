...
fn main() -> io::Result<()>
{
	...
	match args.len()
	{
		...
		2 => {buffer = fs::read(&args[1])?;}, //leitura de todo o ficheiro que esta no args[1], seja um caminho absoluto ou relativo, para o buffer 
...
fn codigo_em_memoria(buffer: Vec<u8>, processor: &mut Processor, mem: &mut HashMap<i32, i32>)
{
	...
	loop
	{
		...
		if i > buffer.len() - 1 {break;}

		//colocar em memoria arranjando, pois o ficheiro encontra-se em Little Endian
		mem.insert(processor.pc, ((buffer[i + 3] as i32) << 24) | ((buffer[i + 2] as i32) << 16) | ((buffer[i + 1] as i32) << 8) | (buffer[i] as i32));
			...
			0b0000011 => match funct_14_12
						{
							...
							0b101 => {
										...
										processor.regs[rd_11_7] = processor.lhu(rs1_19_15, imm_31_20, mem).try_into().unwrap();
									 },
							...
							0b100 => {
										...
										processor.regs[rd_11_7] = processor.lbu(rs1_19_15, imm_31_20, mem).try_into().unwrap();
									 },
